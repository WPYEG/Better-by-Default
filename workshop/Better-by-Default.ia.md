...
[NIST SP 800-63B-4 § 3.1.1.2, Password Verifiers](https://pages.nist.gov/800-63-4/sp800-63b/authenticators/#passwordver) calls for at least 15 characters for single-factor passwords, no composition rules, and a blocklist of commonly used, expected, or compromised passwords. It is worth being precise about *why* composition rules are out, because the usual shorthand — "they do not add entropy" — is not quite true and loses the argument to anyone who checks. Demanding an uppercase letter, a digit and a symbol genuinely does enlarge the alphabet, so on paper it adds entropy. What it fails to add is **guessing resistance**: people satisfy the rule the same few ways, and `Password1!` is the shape a whole population converges on. An attacker's dictionary is built from what people actually chose, not from the keyspace they were theoretically offered. Entropy is also the measure NIST itself stopped using for user-chosen secrets — the word does not appear in § 3.1.1.2 at all. BBD first applies its length rule, a small local blocklist, and checks for the username or email name. It then screens the candidate against the [Have I Been Pwned Pwned Passwords range API](https://haveibeenpwned.com/API/v3#SearchingPwnedPasswordsByRange).
...
BBD also sends `Add-Padding: true`, so response size does not disclose how many real matches exist; synthetic rows have a count of zero and are ignored. WordPress caps the response at 128 KiB with `limit_response_size`. Because a response reaching that cap may be truncated, capped, empty, malformed, failed, and non-200 responses are treated as unavailable and **fail open**. Failing open is a deliberate trade, not an oversight. The alternative is that nobody on the site can change a password while somebody else's API is down — worst precisely when people are rotating credentials after an incident — and the length, blocklist and personal-context rules all still apply, so a failed lookup drops one control rather than all four. The cost is worth naming out loud: anything that stops the site reaching the API turns breach screening off quietly, and a site that has silently stopped screening looks exactly like one that is screening and finding nothing. Only structurally valid prefix responses are cached for 12 hours. The local length, blocklist, and personal-context checks still apply. The same server-side validator covers profile changes, password resets, and REST user-password requests.
